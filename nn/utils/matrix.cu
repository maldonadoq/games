...
Matrix::Matrix(size_t x_dim, size_t y_dim) : shape(x_dim, y_dim), data_device(nullptr), data_host(nullptr),
...
Matrix::Matrix(Shape shape, std::vector<float> input) : Matrix(shape.x, shape.y){
	allocateMemory();
