...
LinearSoftmaxLayer::LinearSoftmaxLayer(std::string name, Shape W_shape) : W(W_shape), b(W_shape.y, 1)
{
	this->name = name;
	b.allocateMemory();
	W.allocateMemory();
...
__global__ void softmax_linear(float *softmaxP, float *b, int rows, int cols)
{
	...
	if (tid * cols + bid < rows * cols)
	{
		for (int i = 0; i < rows; i++)
			_share[i] = b[i * cols + bid];
		for (int i = 0; i < rows; i++)
			_max = max(_max, _share[i]);
		for (int i = 0; i < rows; i++)
			_share[i] = __expf(_share[i] - _max);
		for (int i = 0; i < rows; i++)
			sum += _share[i];
		for (int i = 0; i < rows; i++)
			softmaxP[i * cols + bid] = _share[i] / sum;
	}
}
