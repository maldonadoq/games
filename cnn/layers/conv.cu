...
void operator_conv(const Tensor *inputs, Tensor *filters, Tensor *cols,
									 const int pad_h, const int pad_w, const int stride_h,
									 const int stride_w, Tensor *output) {
...
void operator_d_conv(
		Tensor *outputs_grad, const Tensor *inputs, const Tensor *cols,
		Tensor *filters, const int pad_h, const int pad_w, const int stride_h,
		const int stride_w, Tensor *filters_grad, Tensor *inputs_grad,
...
void operator_conv_bias(const Tensor *inputs, const Tensor *bias,
												Tensor *output) {
...
void operator_d_conv_bias(
		const Tensor *outputs_grad, Tensor *bias_grad,
		std::unordered_map<std::string, std::unique_ptr<Tensor>> &temp) {
	// N*C*H*W ==> 1*C
...
Conv::Conv(int height, int width, int channel_in, int channel_out, int kernel_h,
	...
	this->filters.reset(
			new Tensor({channel_out, channel_in, kernel_h, kernel_w}));
	...
	if (is_bias) {
		this->bias.reset(new Tensor({1, channel_out}));
		this->bias_grad.reset(new Tensor({1, channel_out}));
		this->bias->xavier(channel_in * height * width,
											 channel_out * height_out * width_out);
	}
}

std::vector<std::pair<Tensor *, Tensor *>> Conv::parameters() {
	if (this->is_bias) {
