...
__global__ void operator_softmax_h(const float *input1, float *output,
...
void operator_softmax(const Tensor *input1, int dim, Tensor *outputs) {
	// input
	...
	operator_softmax_h<<<grid_size, BLOCK_SIZE, shared_memory_size>>>(
...
__global__ void operator_d_softmax_h(const float *output_grads,
																				 const float *input1,
...
// Y = softmax(X) = x - log(exp(X) * 1_n) * 1_n^T
// dL/dX = dL/dY - (dL/dY * 1_n * exp(x)) / (exp(x) * 1_n)
void operator_d_softmax(const Tensor *output_grads, const Tensor *input1,
														int dim, Tensor *inputs_grad) {
	const float *input1_ptr = RAW_PTR(input1->get_data());
	thrust::device_vector<int> input_shape = input1->get_shape();
	const int *input1_shape_ptr = RAW_PTR(input_shape);
	...
	operator_d_softmax_h<<<grid_size, BLOCK_SIZE, shared_memory_size>>>(
...
void Softmax::forward() {
	...
	operator_softmax(input, this->dim, this->output.get());
}
...
void Softmax::backward() {
	...
	operator_d_softmax(output_grad, input, this->dim, this->grad.get());
}